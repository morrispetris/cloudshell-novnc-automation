...
#screen -d -m docker run -p 8080:6080 -e RESOLUTION=1920x1080 -v /dev/shm:/dev/shm liwanggui/ubuntu-desktop-novnc

#screen -d -m  docker run \
#  --name desktop \
#  -p 8080:80 \
...
screen -d -m docker run \
  ...
  -e RESOLUTION=1920x1080 \
  -v /dev/shm:/dev/shm \
  -v /mnt/nfs-share:/mnt/nfs-share \
  -v ~/cloudshell-novnc-automation:/workspace \
  --device /dev/fuse \
  --cap-add SYS_ADMIN \
